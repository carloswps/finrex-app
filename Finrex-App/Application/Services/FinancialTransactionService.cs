...
public class FinancialTransactionService : IFinancialTransactionService
{
    ...
    public async Task<List<TopSavingsMonth>> GetTopSavingsMonthAsync(int userId)
    {
        ...
        var savingsList =
            BalanceHelperMonthlySavings.CalculateMonthlySavings(allMonthSpending.Select(x => (x.Month, x.TotalSpending))
                .ToList());
    ...
    public async Task<SpendingSummaryDto?> GetCurrentMonthSpendingSummaryAsync(int userId)
    {
        ...
        if (spendingSummary == null) return new SpendingSummaryDto { Period = startOfMonth.ToString("yyy-MM") };

        return spendingSummary;
    }

    public async Task<List<MIncomeResponseDto>> GetIncomeAsync(int userId)
    {
        var income = await _context.MIncome
            .Where(i => i.UsuarioId == userId)
            .ToListAsync();

        return income.Select(i => _mapper.Map<MIncomeResponseDto>(i)).ToList();
    }

    public async Task<MIncomeResponseDto> GetIncomeByIdAsync(int id, int userId)
    {
        var incomeById = await _context.MIncome
            .Where(i => i.Id == id && i.UsuarioId == userId)
            .FirstOrDefaultAsync();

        if (incomeById == null) return null;

        return _mapper.Map<MIncomeResponseDto>(incomeById);
    }

    public async Task<bool> UpdateIncomeAsync(int id, MIncomeDto dto, int userId)
    {
        var income = await _context.MIncome.FirstOrDefaultAsync(i => i.Id == id && i.UsuarioId == userId);
        if (income == null) return false;

        income.Date = dto.Date;
        income.MainIncome = dto.MainIncome ?? 0;
        income.Freelance = dto.Freelance ?? 0;
        income.Benefits = dto.Benefits ?? 0;
        income.BusinessProfit = dto.BusinessProfit ?? 0;
        income.Other = dto.Other ?? 0;
    ...
    public async Task<bool> DeleteIncomeAsync(int id, int userId)
    {
        ...
        _context.MIncome.Remove(income);
        await _context.SaveChangesAsync();
        return true;
    }
